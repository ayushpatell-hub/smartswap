...
function OtpInput({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChange = useCallback(
    (index: number, value: string) => {
      ...
      // Handle paste — if value is more than 1 char
      if (value.length > 1) {
          ...
          <input
            ...
            className={`w-12 h-14 text-center text-xl font-bold rounded-xl border-2 transition-all duration-200 outline-none bg-[#1a2235] text-white
              ${digit
                ...
                : "border-white/10 focus:border-yellow-400/60 focus:bg-yellow-400/5"
              }`}
          />
        ))}
      </div>
        ...
        {otp.map((digit, i) => (
          <div
            ...
            className={`h-1 rounded-full transition-all duration-300 ${
              digit ? "w-6 bg-yellow-400" : "w-2 bg-white/10"
            }`}
          />
        ))}
      </div>
    ...
    <div className="min-h-screen bg-[#080c14] flex flex-col items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 mb-1">
            ...
            <span className="text-white font-bold text-2xl tracking-wide">
              Smart<span className="text-yellow-400">Swap</span>
            </span>
          </div>
        ...
        <div className="bg-[#0f1623] border border-white/10 rounded-2xl p-8 shadow-2xl">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="bg-[#1a2235] border border-white/10 rounded-xl px-3 flex items-center text-gray-300 text-sm min-w-[72px] justify-center">
                    🇮🇳 +91
                  </div>
              ...
              <button onClick={() => regValid && setScreen("register-otp")}
              ...
              <p className="text-center text-gray-500 text-sm">
                ...
                <button onClick={() => setScreen("login")} className="text-yellow-400 font-semibold hover:underline">
                  Sign In
                </button>
              </p>
              ...
              <p className="text-center text-gray-600 text-[10px]">Some features require an account</p>
            </div>
          )}
          ...
          {screen === "register-otp" && (
            <div className="space-y-6">
              <button onClick={() => setScreen("register")}
                className="text-gray-500 text-sm hover:text-gray-300 flex items-center gap-1 transition-all">
                ← Back
              </button>
              <div>
                <h2 className="text-white text-xl font-bold mb-1">Verify Phone 🔐</h2>
                <p className="text-gray-400 text-sm">
                  OTP sent to <span className="text-white font-medium">+91 {reg.phone}</span>
                </p>
              </div>

              <div className="bg-yellow-400/10 border border-yellow-400/20 rounded-xl px-4 py-3">
                <p className="text-yellow-400 text-xs font-medium">💡 Type any 6 digits — auto moves box to box. Paste works too!</p>
              </div>

              <div>
                <label className="text-gray-300 text-sm font-medium block mb-4">Enter 6-digit OTP</label>
                <OtpInput onComplete={goHome} />
              </div>

              <button onClick={goHome}
                className="w-full py-3 bg-yellow-400 text-black rounded-xl font-bold text-sm hover:bg-yellow-300 transition-all shadow-lg shadow-yellow-400/25">
                Verify & Create Account ✓
              </button>

              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-500">Didn't receive OTP?</span>
                <button className="text-yellow-400 hover:underline font-medium">Resend OTP</button>
              </div>
            </div>
          )}
          ...
          {screen === "login" && (
            <div className="space-y-5">
              ...
              {loginMethod && (
                <button onClick={() => loginValid && setScreen("login-otp")}
                  className={`w-full py-3 rounded-xl font-bold text-sm transition-all duration-300 ${
                    loginValid
                      ? "bg-yellow-400 text-black hover:bg-yellow-300 shadow-lg shadow-yellow-400/25"
                      : "bg-yellow-400/20 text-yellow-400/40 cursor-not-allowed"
              ...
              <p className="text-center text-gray-500 text-sm">
                New to SmartSwap?{" "}
                <button onClick={() => setScreen("register")} className="text-yellow-400 font-semibold hover:underline">
                  Create Account
                </button>
              </p>
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  ...
                  <span className="text-white font-medium">
                    {loginMethod === "phone" ? `+91 ${loginPhone}` : loginEmail}
                  </span>
                </p>
              </div>
        ...
        <p className="text-center text-gray-600 text-xs mt-6">
          🔐 256-bit encrypted · RBI compliant · Made in India 🇮🇳
        </p>
      </div>
